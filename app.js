import { addReading, deleteReading, clearAll, getAllReadings, getReadingById, updateReading, importReadings } from './db.js';
...
function parseCSV(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const reading = {
      ...
      timestamp: parseInt(obj.timestamp_ms) || new Date(obj.timestamp_iso).getTime(),
...
function showImportModal(readings) {
  ...
  const moreText = readings.length > 5
